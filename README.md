...
- 修复 Qwen Token Plan 报 `403 invalid api-key`：Token Plan 有**专属 Anthropic 接入点** `https://token-plan.cn-beijing.maas.aliyuncs.com/apps/anthropic`（官方 Claude Code 接入参数确认），`sk-sp-` 订阅 Key 打到按量付费的 `dashscope.aliyuncs.com/apps/anthropic` 会被拒。v1.11.1 中"与按量付费同一端点"的说法有误，已更正 `qwen-plan` 的 `base_url` 及相关注释；Bearer 与 x-api-key 两种鉴权头实测均可用
...
- 新增 **Qwen Token Plan 订阅** provider（`qwen-plan-3.8-max`，别名 `qwen-plan`）：Key 来自 [千问 AI 平台](https://platform.qianwenai.com/) 的 `sk-sp-` 订阅密钥，需配置 `QWEN_PLAN_API_KEY`；走 DashScope 同一 Anthropic 兼容端点（`dashscope.aliyuncs.com/apps/anthropic`），但按订阅额度计费，另含 `qwen-plan-3.7-max`
