    ...
    for (const port of ports) {
      ...
      switch (result.status) {
        ...
        case 'unverified_gateway':
          ...
          console.error(
            '  If it is yours (e.g. started with a different CCMR_HOME), stop it manually:'
          );
  ...
  .action(async (prompt, options, command) => {
    ...
    if (!sourceCheck.ok) {
      ...
      if (sourceCheck.reason === 'source_unknown') {
        console.error(
          `\x1b[31m[ERROR]\x1b[0m The gateway on port ${gatewayPort} was started by an older ccmr ` +
        ...
        console.error(
          `  Current config: ${configManager.getConfigFilePath() ?? 'built-in defaults'}`
        );
    ...
    if (!readiness.ok) {
      ...
      console.error('');
      const usable = availableModels(gateway.health);
